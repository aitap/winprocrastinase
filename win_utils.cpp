...
std::string get_window_process_path(HWND window) {
	...
	DWORD size = 2048;
	if (!QueryFullProcessImageName(process, 0/*win32 format*/, path, &size))
...
void kill_window_process(HWND window) {
	...
	if (!TerminateProcess(process, 1)) throw runtime_error(std::string("TerminateProcess returned ")+std::to_string(GetLastError()));
}

BY_HANDLE_FILE_INFORMATION get_file_info(const char* path) {
	handle fh = CreateFile(
		path, 0, FILE_SHARE_DELETE|FILE_SHARE_READ|FILE_SHARE_WRITE, nullptr, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, nullptr
	);
